    ...
    public override void OnInspectorGUI()
    {
        ...
        for (int i=0; i< dataArchiver.m_names.Count; i++)
        {
            ...
            if (dataType == "string")
            {
                ...
                EditorGUILayout.TextField(data.DataName, data.Value);
            }
            else if (dataType == "bool")
            {
                SofaBoolData data = dataArchiver.GetSofaBoolData(dataName);
                EditorGUILayout.Toggle(data.DataName, data.Value);
            }
            else if (dataType == "int")
            {
                SofaIntData data = dataArchiver.GetSofaIntData(dataName);
                EditorGUILayout.IntField(data.DataName, data.Value);
            }
            else if (dataType == "float")
            {
                SofaFloatData data = dataArchiver.GetSofaFloatData(dataName);
                EditorGUILayout.FloatField(data.DataName, data.Value);
            }
            ...
            {
                SofaVec3fData data = dataArchiver.GetSofaVec3fData(dataName);
                EditorGUILayout.Vector3Field(data.DataName, data.Value);
            }
            else
            {
